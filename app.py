...
# Voorbeeldtranscript om de analyse te testen zonder Voys/opname.
_VOORBEELD_TRANSCRIPT = (
    "Participant 1: Goedemiddag, u spreekt met de klantenservice van Toteco.\n"
    "Participant 2: Hallo, mijn thuisbatterij staat sinds de installatie vorige week "
    "offline. Het lampje knippert rood en in de app zie ik niets.\n"
    "Participant 1: Vervelend, dat snap ik. Ik plan een monteur voor u in en mail de "
    "resetinstructies alvast.\n"
    "Participant 2: Graag, want zo heb ik er niets aan. Ik verwacht wel dat het snel "
    "opgelost wordt."
)


@app.route("/api/test-analyse", methods=["POST"])
def test_analyse():
    """
    TIJDELIJK: test de analyse-keten (Claude + validatie) op een voorbeeld-
    transcript, ZONDER Voys en ZONDER opslag in het rapport. Zo zie je dat de
    onderwerp-indeling en het sentiment werken, ook als er niemand opneemt.
    """
    body = request.get_json(silent=True) or {}
    transcript = body.get("transcript") or _VOORBEELD_TRANSCRIPT
    try:
        ruw, prompt_versie, model = analyzer.analyseer_transcript(transcript)
        schoon = validation.valideer_analyse(ruw)
        return jsonify({"ok": True, "analyse": schoon,
                        "prompt_versie": prompt_versie, "model": model})
    except validation.ValidatieFout as e:
        return jsonify({"ok": False, "fout": "validatie", "detail": str(e)})
    except Exception as e:  # noqa: BLE001
        return jsonify({"ok": False, "fout": "ai", "detail": str(e)})
...
<div class="rij">
  <button class="secondary" onclick="testAnalyse()">3. Test analyse met voorbeeldgesprek (geen Voys nodig)</button>
</div>
...
async function testAnalyse(){
  toon("Voorbeeldgesprek analyseren met Claude...");
  try{
    const r = await fetch("/api/test-analyse", {method:"POST"});
    const d = await r.json();
    toon(JSON.stringify(d, null, 2));
  }catch(e){ toon("Fout: " + e); }
}
